"""Air India Express (airindiaexpress.com) Asynchronous Playwright Scraper.
Carrier: Air India Express (IATA: IX)
"""
import argparse
import asyncio
import datetime
...
logger = logging.getLogger("scraper.air_india_express")
...
def extract_air_india_express_quotes_from_dom_or_html(
    html_text: str,
    origin: str,
    destination: str,
    ...
    flight_blocks = re.findall(
        r'(<div[^>]*class="[^"]*(?:flight-tile|fare-row|express-flight-card|flight-card)[^"]*"[^>]*>.*?</div>\s*</div>)',
        html_text,
        re.DOTALL | re.IGNORECASE,
    )
    if not flight_blocks:
        flight_blocks = re.findall(
            r'(<div[^>]*data-testid="flight-tile"[^>]*>.*?</div>\s*</div>)',
            html_text,
            re.DOTALL | re.IGNORECASE,
        )

    for block in flight_blocks:
        # Flight Number: IX-xxx
        fn_match = re.search(r'(?:IX[- ]?(\d{3,4}))|(?:flight-no[^>]*>\s*([A-Z0-9 -]+))', block, re.IGNORECASE)
        flight_num = "IX-000"
        if fn_match:
            flt = fn_match.group(1) or fn_match.group(2)
            flt_clean = flt.strip().replace(" ", "-")
            flight_num = flt_clean if flt_clean.startswith("IX") else f"IX-{flt_clean}"

        # Sold Out check
        is_sold_out = bool(re.search(r'(?:sold\s*out|not\s*available|unavailable|sold-out-badge)', block, re.IGNORECASE))
        ...
        # Fare classes: Xpress Lite, Xpress Value, Xpress Flex
        fare_classes = ["Xpress Flex", "Xpress Value", "Xpress Lite", "Economy"]
        matched_class = "Xpress Lite"
        ...
        total_match = re.search(r'(?:total[\s_-]*fare|final[\s_-]*fare|amount)[^>]*>.*?(?:₹|INR|Rs\.?)\s*([\d,]+(?:\.\d{2})?)', block, re.IGNORECASE)
        if total_match:
...
class AirIndiaExpressScraper(BaseScraper):
    ...
    async def extract_fares(
        ...
        search_url = (
            ...
            f"?origin={origin}&dest={destination}&date={date_str}&trip=OW&adults=1"
        )
        logger.info("[Air India Express] Navigating to %s", search_url)

        try:
            await page.goto(search_url, wait_until="networkidle", timeout=self.default_timeout_ms)
        except Exception as exc:
            logger.warning("[Air India Express] Initial goto networkidle timed out (%s), continuing...", exc)

        selectors = [
            ".flight-tile",
            ".fare-row",
            "[data-testid='flight-tile']",
            ".express-flight-card",
        ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Air India Express Scraper")
    ...
    scraper = AirIndiaExpressScraper(demo_mode=args.demo, headless=args.headless)
    ...
    print(f"Air India Express Scrape Complete: {len(results)} quotes collected.")

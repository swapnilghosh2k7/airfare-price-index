"""SpiceJet (spicejet.com) Asynchronous Playwright Scraper.
Carrier: SpiceJet (IATA: SG)
"""
import argparse
import asyncio
import datetime
...
logger = logging.getLogger("scraper.spicejet")
...
def extract_spicejet_quotes_from_dom_or_html(
    html_text: str,
    origin: str,
    destination: str,
    ...
    flight_blocks = re.findall(
        r'(<div[^>]*class="[^"]*(?:spice-flight-row|flight-card|css-1dbjc4n\s*r-14lw9ot|r-13awgt0)[^"]*"[^>]*>.*?</div>\s*</div>)',
        html_text,
    ...
    for block in flight_blocks:
        # Flight Number: SG-xxx
        fn_match = re.search(r'(?:SG[- ]?(\d{3,4}))|(?:flight-number[^>]*>\s*([A-Z0-9 -]+))', block, re.IGNORECASE)
        flight_num = "SG-000"
        if fn_match:
            flt = fn_match.group(1) or fn_match.group(2)
            flt_clean = flt.strip().replace(" ", "-")
            flight_num = flt_clean if flt_clean.startswith("SG") else f"SG-{flt_clean}"

        # Sold Out check
        is_sold_out = bool(re.search(r'(?:sold\s*out|sold-out|unavailable|no\s*seats)', block, re.IGNORECASE))
        status = "SOLD_OUT" if is_sold_out else "AVAILABLE"

        # Seats remaining
        seats_match = re.search(r'(\d+)\s*(?:seats?\s*left|remaining|seats?\s*available)', block, re.IGNORECASE)
        seats_remaining = int(seats_match.group(1)) if seats_match else None

        # Fare classes: Spicesaver, SpiceMax
        fare_classes = ["SpiceMax", "Spicesaver", "Economy"]
        matched_class = "Spicesaver"
        ...
        total_match = re.search(r'(?:total[\s_-]*fare|final[\s_-]*fare|flight-price|price)[^>]*>.*?(?:₹|INR|Rs\.?)\s*([\d,]+(?:\.\d{2})?)', block, re.IGNORECASE)
        if total_match:
...
class SpiceJetScraper(BaseScraper):
    ...
    async def extract_fares(
        ...
        logger.info("[SpiceJet] Navigating to %s", search_url)

        try:
            await page.goto(search_url, wait_until="networkidle", timeout=self.default_timeout_ms)
        except Exception as exc:
            logger.warning("[SpiceJet] Initial goto networkidle timed out (%s), continuing...", exc)
...
if __name__ == "__main__":
    ...
    scraper = SpiceJetScraper(demo_mode=args.demo, headless=args.headless)
    ...
    print(f"SpiceJet Scrape Complete: {len(results)} quotes collected.")
